...
- 貪心的題目常出現要求一種排序或集合，使用枚舉的時間複雜度為 $O(N!)$ 和 $O(N2^N)$，如果 $N$ 過大，使用枚舉就會 TLE
...
- 暴力
    ...
    - $O(2^N)$
- 貪心
    - 從左到右掃描稻田，如果第 $i$ 塊稻田有作物，就把稻草人放到第 $i+1$ 塊稻田，這樣能保護第 $[i,i+2]$ 塊稻田，接著從第 $i+3$ 塊稻田掃描
    - $O(N)$
...
- 給定一張高 $L$ 寬 $W$ 的草坪，有多個灑水器在草坪正中間，假設灑水器的位置和半徑為 $(x_i,\frac{L}{2})$ 和 $r_i$，問至少要擠個灑水器，才能讓整個草坪都灑滿水。

<v-clicks>

- 這是先算出每個灑水器與矩形上下兩邊的交界 $(L_i,R_i)$，題目就轉成最少區間覆蓋問題。
